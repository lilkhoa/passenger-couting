...
def main():
    ...
    f_csv = open("metrics_report_96RGB.csv", "w")
    f_csv.write("Video,Frames,UP,DOWN,AvgFPS,AvgLat_ms,PeakRAM_KB,Flash_KB,TotalEnergy_J,AvgCPU%\n")
    ...
    for video_path in video_files:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                if line.startswith('{'):
                    ...
                    ram = data.get('ram_used', 0) / 1024.0
                    flash = data.get('flash_used', 0) / 1024.0
                    ...
                    print(f"\rFr {frames_count:<4}| FPS:{fps:.1f}| RAM:{ram:.0f}KB| CPU:{cpu_usage:.1f}%| E:{energy:.1f}mJ| UP:{last_up} DWN:{last_down}", end="")
            except: pass
        ...
        f_csv.write(f"{os.path.basename(video_path)},{frames_count},{last_up},{last_down},{avg_fps:.2f},{avg_lat:.2f},{ram_peak:.2f},{flash_size:.2f},{total_energy_joules:.4f},{avg_cpu:.2f}\n")
        f_csv.flush()

        print(f"\nDone: {avg_fps:.2f} FPS | {total_energy_joules:.2f} J | RAM: {ram_peak:.0f} KB | CPU: {avg_cpu:.1f}%")
